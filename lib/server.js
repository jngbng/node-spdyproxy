...
var SPDYProxy = function(options) {
  ...
  this.setAuthHandler = function(handler) {
    ...
    logOnVerbose('AuthHandler'.green, handler.friendly_name.yellow,
                'will be used.'.green);
  }

  this.setLogHandler = function(handler) {
    self._logHandler = handler;
    logOnVerbose('Requests will be logged into file'.green, handler._filename.yellow);
  }

  function logRequest(req) {
    logOnVerbose(req.method.green + ' ' + req.url.yellow);
    for (var i in req.headers)
      logOnVerbose(' > '.grey + i.cyan + ': ' + req.headers[i]);
    logOnVerbose();
  }
  ...
  function handlePlain(req, res) {
    ...
    rreq.on('error', function(e) {
      logOnVerbose("Client error: " + e.message);
  ...
  function handleSecure(req, socket) {
    ...
    tunnel.on('error', function(e) {
      logOnVerbose("Tunnel error: ".red + e);
  ...
  function handleRequest(req, res) {
    ...
    logOnVerbose("%s:%s".yellow + " - %s - " + "stream ID: " + "%s".yellow + " - priority: " + "%s".yellow,
